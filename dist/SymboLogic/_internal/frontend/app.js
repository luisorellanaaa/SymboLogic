...
const handleAuth = (e) => {
    ...
    if (authMode === 'register') {
        ...
        gameData.users[username] = { password: password, role: 'user', coins: 5 };
        saveData();
...
const handleAnswer = (isCorrect) => {
    if (isCorrect) {
        ...
        showNotification('+10 Puntos | +1 🪙', 'success');
        document.getElementById('current-score').textContent = score;
        nextQuestion();
    } else {
        timeLeft -= 5;
...
const usePower = (type) => {
    ...
    if (type === '5050') {
        ...
        showNotification('-2 🪙 | 50/50 Activado', 'success');

    } else if (type === 'time') {
        gameData.users[currentUser].coins -= cost;
        timeLeft += 15;
        document.getElementById('time-display').textContent = timeLeft;
        showNotification('-1 🪙 | +15 Segundos', 'success');

    } else if (type === 'skip') {
        gameData.users[currentUser].coins -= cost;
        showNotification('-3 🪙 | Pregunta Saltada', 'success');
        nextQuestion();
    }
